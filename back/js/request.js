...
async function requestPDC(id_pdc, type_prise) {
    try {
        // Ajout explicite de "/pdcs" pour respecter le routage du fichier PHP
        const response = await fetch(`../../utils/request.php/pdcs?id_pdc=${id_pdc}&type_prise=${type_prise}`, {
            method: 'GET'
        });
        const pdcs = await response.json();
        const pdc = pdcs[0]; // On récupère le premier élément
...
// Si on est sur la page avec le tableau des pdcs (liste)
if (document.getElementById('pdcs-table-body')) {
    requestPDCS();
}

// Si on est sur la page de détail (qui a besoin du l'id et du type_prise)
