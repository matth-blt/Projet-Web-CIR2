...
/**
 * ============================================================================
 *  bdd.php  —  Réécriture fidèle de bdd.py (pandas + SQLAlchemy) en PHP / PDO
 * ============================================================================
 *
 *  Le script Python d'origine s'appuyait sur pandas pour toute la manipulation
 *  de données. PHP n'ayant pas d'équivalent, les opérations pandas sont ici
 *  reproduites « à la main » via des fonctions utilitaires (lecture CSV,
 *  drop_duplicates, dropna, merge, to_numeric, astype(str), troncage str[:n]…).
 *
 *  Exécution :  php bdd.php
 *  (les fichiers irve_init.csv et communes-france-2024-limite.csv doivent se
 *   trouver dans le répertoire courant, comme dans le script Python.)
 * ============================================================================
 */
...
try {
    ...
    foreach ($df as $r) {
        ...
        if ($c === null) continue;
        if (isset($seen_codes[$c])) continue;
        $seen_codes[$c] = true;
        $all_irve_codes[] = $c;
    }
    ...
    // ==========================================================================
    // 4. ENTITÉ UNIQUE : ACTEUR (AMÉNAGEURS & OPÉRATEURS)
    // ==========================================================================
    ...
    foreach ($df as $r) {
        ...
        if (is_na($siren_raw)) continue;
        $key = (string)$siren_raw;
        if (isset($seen_siren[$key])) continue;
        $seen_siren[$key] = true;
        $df_am[] = [
    ...
    foreach ($df as $r) {
        ...
        if (is_na($nom)) continue;
        $key = (string)$nom;
        if (isset($seen_op[$key])) continue;
        $seen_op[$key] = true;
        $df_op[] = [
            'nom_acteur'       => $nom,
    ...
    insert_rows($pdo, 'acteur',
        ['siren_acteur', 'nom_acteur', 'contact_acteur', 'role_acteur', 'telephone_acteur'],
        $df_acteurs_all);

    // Relecture pour récupérer les id_acteur générés
    $db_acteurs = $pdo->query('SELECT id_acteur, siren_acteur, nom_acteur, role_acteur FROM acteur')
                      ->fetchAll();
    ...
    foreach ($df as $r) {
        $id = $r['id'] ?? null;
        if (is_na($id)) continue;                       // dropna(id_pdc)
        $key = (string)$id;
        if (isset($seen_pdc[$key])) continue;           // drop_duplicates(id_pdc)
        $seen_pdc[$key] = true;
        ...
        $pdc_rows[] = [
            ...
            'lat'              => $lat,
            'puissance'        => $pui,
            'cable_t2_attache' => $r['cable_t2_attache'] ?? 0,   // déjà nettoyé en 0/1
            'gratuit'          => $r['gratuit'] ?? 0,            // déjà nettoyé en 0/1
    ...
    foreach ($df as $r) {
        $h = $r['horaires'] ?? null;
        $sid = $r['id_station_itinerance'] ?? null;
        if (is_na($h) || is_na($sid)) continue;          // dropna()
        $k = $h . '|' . $sid;
        if (isset($seen[$k])) continue;                  // drop_duplicates()
        $seen[$k] = true;
        $a_une[] = [
    ...
    foreach ($df as $r) {
        $id  = $r['id'] ?? null;
        $sid = $r['id_station_itinerance'] ?? null;
        if (is_na($id) || is_na($sid)) continue;         // dropna()
        $k = $id . '|' . $sid;
        if (isset($seen[$k])) continue;                  // drop_duplicates()
        $seen[$k] = true;
        $possede_des[] = [
    ...
    foreach ($df as $r) {
        $id = $r['id'] ?? null;
        if (is_na($id)) continue;                        // dropna(subset=['id'])
        $k = (string)$id;
        if (isset($seen_id[$k])) continue;               // drop_duplicates(subset=['id'])
        $seen_id[$k] = true;
