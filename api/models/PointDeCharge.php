        ...
        public function getMapPoints(array $filters): array {
            try {
                $request = '
                    SELECT 
                        pdc.id_pdc AS id, 
                        MAX(s.nom_station) AS nom_station,
                        MAX(s.adresse_station) AS adresse_station,
                        MAX(c.nom_commune) AS localite, 
                        MAX(c.code_dep) AS dept, 
                        MAX(YEAR(s.date_mise_en_service)) AS annee, 
                        MAX(pdc.puissance) AS puissance, 
                        MAX(pdc.lat) AS lat, 
                        MAX(pdc.lon) AS lng,
